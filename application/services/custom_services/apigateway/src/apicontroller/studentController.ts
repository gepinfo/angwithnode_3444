...
this.router.get('/student/:id', this.GpGetEntityById);
...
public GpDelete(req: Request, res: Response) {
        ...
        new ApiAdapter().delete(Constant.STUDENTMANAGEMENTURL + `${req.url}` )
...
public GpUpdate(req: Request, res: Response) {
        ...
        new ApiAdapter().put(Constant.STUDENTMANAGEMENTURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
...
public GpGetEntityById(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into studentController.ts: GpGetEntityById');
        new ApiAdapter().get(Constant.STUDENTMANAGEMENTURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
            new CustomLogger().showLogger('info', 'Exit from studentController.ts: GpGetEntityById');
        }).catch(err => {
            res.send(err);
        });
    }
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdapter().post(Constant.STUDENTMANAGEMENTURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
