...
public GetEntityById(req: Request, res: Response) {
    ...
    student.GetEntityById(req, (response) => {
                new CustomLogger().showLogger('info', 'Enter into studentController.ts: GetEntityById');
     res.status(200);
     res.json(response);
                new CustomLogger().showLogger('info', 'Exit from studentController.ts: GetEntityById');
    })}
