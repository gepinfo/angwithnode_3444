...
public async GetEntityById(studentId, callback){
    
    new CustomLogger().showLogger('info', 'Enter into studentDao.ts: GetEntityById');
    ...
    this.student.findById(studentId).then((result)	=>
        ...
        new CustomLogger().showLogger('info', 'Exit from studentDao.ts: GetEntityById');
