...
public  GetEntityById(req: Request, callback){
    new CustomLogger().showLogger('info', 'Enter into studentService.ts: GetEntityById')
     let  studentId = req.params.id;
     student.GetEntityById(studentId,(response)=>{
             new CustomLogger().showLogger('info', 'Exit from studentService.ts: GetEntityById')
